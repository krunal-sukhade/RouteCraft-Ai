...
export default function Chat() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, role: "bot", text: "Ask me to plan your next trip with dates and budget." },
    { id: 2, role: "user", text: "Plan a 2-day Pune trip with food spots and forts." },
    { id: 3, role: "bot", text: "Great choice. I built a compact route with travel time in mind." },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const normalizedReply = data.reply.replace(/\s+/g, " ").trim();
      if (!normalizedReply) {
  ...
  return (
    <div className="min-h-screen bg-transparent pt-20 md:pt-24">
      <div className="h-[calc(100vh-80px)] overflow-hidden p-4 md:p-5">
            ...
            <section className="grid min-h-0 min-w-0 gap-4 md:gap-5 md:grid-rows-[250px_minmax(300px,1fr)]">
              ...
              <div className="grid min-h-0 grid-rows-[1fr_auto] gap-4 rounded-3xl border border-blue-200/15 bg-slate-900/50 p-4 shadow-[0_18px_70px_rgba(3,10,26,0.6)] backdrop-blur-xl md:p-5">
                <div className="grid min-h-0 gap-3 overflow-y-auto pr-1">
                  {messages.map((msg) => (
                    <p
                      ...
                      className={`w-fit max-w-[85%] rounded-2xl px-4 py-3 text-sm ${
                        msg.role === 'user'
                          ? "justify-self-end rounded-br-sm bg-gradient-to-r from-cyan-400 to-blue-500 font-medium text-slate-900"
                          : "rounded-bl-sm border border-white/15 bg-slate-800/85 text-slate-100"
                      }`}
                    >
                      {msg.text}
                    </p>
                  ...
                  {isLoading && (
                    <p className="w-fit max-w-[85%] rounded-2xl rounded-bl-sm border border-white/15 bg-slate-800/85 px-4 py-3 text-sm text-slate-100">
                      Thinking...
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-[1fr_auto_auto] gap-2">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-amber-300 to-orange-500 text-slate-900 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0"
                    type="button"
